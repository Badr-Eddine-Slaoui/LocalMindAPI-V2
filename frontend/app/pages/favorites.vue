        ...
        <main class="flex-1 max-w-[1200px] mx-auto flex flex-col lg:flex-row gap-8 px-4 lg:px-10 py-8 w-full">
            <aside class="hidden xl:flex flex-col w-64 gap-6 shrink-0">
                    ...
                    <div class="pt-8 px-5 pb-5">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="text-center p-2 bg-white/5 rounded-lg border border-white/5">
                                <span
                                    class="block text-primary font-bold text-lg">{{ auth?.user?.favorites?.length }}</span>
                                <span class="text-[10px] text-white/40 font-bold uppercase">Favorites</span>
                            </div>
                            <div class="text-center p-2 bg-white/5 rounded-lg border border-white/5">
                                <span
                                    class="block text-primary font-bold text-lg">{{ auth?.user?.questions?.length }}</span>
                ...
                <div class="bg-sidebar-black p-5 rounded-xl border border-white/5">
                    ...
                    <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                        <div :class="`h-full bg-primary w-[${auth?.user?.favorites?.length}%]`"></div>
                    </div>
                    <p class="text-[10px] text-white/30 mt-2 font-bold uppercase">{{ auth?.user?.favorites?.length }}/100
                        Slots Filled</p>
                </div>
            </aside>
            
            <div v-if="store?.favorites?.length as number > 0" class="flex-1 flex flex-col">
                    ...
                    <div v-for="favorite in store.favorites"
                        ...
                        <div>
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <form @submit.prevent="store.unfavorite(favorite?.question?.id)" method="post">
                        ...
                        <div class="flex items-center justify-between mt-auto">
                            ...
                            <NuxtLink :to="`/questions/${favorite?.question?.id}`"
            ...
            <div v-else
                ...
                <div class="relative z-10 flex flex-col items-center text-center max-w-md px-6">
                    ...
                    <NuxtLink to="/" class="inline-flex items-center justify-center gap-3 bg-primary hover:bg-primary/90 text-background-deep font-black px-10 py-4 rounded-xl text-lg shadow-[0_0_20px_rgba(251,191,36,0.2)] transition-all hover:scale-105 active:scale-95 group">
