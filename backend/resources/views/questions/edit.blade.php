...
@section('content')
    <main class="flex-1 max-w-5xl mx-auto w-full p-6 sm:p-12 z-10">
        <div class="mb-8">
            <h1 class="text-4xl font-extrabold text-white mb-2 neon-text-gold uppercase italic">Edit Bounty
                Question</h1>
            ...
            <form action="{{ route('questions.update', $question->id) }}" method="POST" class="space-y-8">
                ...
                <div class="space-y-3">
                    ...
                    <input name="title" value="{{ old('title', $question->title) }}"
                        ...
                        placeholder="What is the secret of the Poneglyphs?" required="" type="text" />
                    @error('title')
                ...
                <div class="space-y-3">
                    ...
                    <div class="editor-bg rounded-xl overflow-hidden border border-cyan-900/30">
                        <textarea name="description"
                            ...
                            placeholder="Describe your query in detail for the crew members...">{{ old('description', $question->description) }}</textarea>
                    </div>
                    @error('description')
                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                    @enderror
