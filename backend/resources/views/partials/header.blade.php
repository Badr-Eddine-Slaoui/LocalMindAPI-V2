    ...
    <div class="max-w-[1200px] mx-auto px-4 lg:px-10 py-3 flex items-center justify-between">
        <div class="flex items-center gap-8">
            ...
            <nav class="hidden md:flex items-center gap-6">
                <a class="text-primary text-sm font-bold border-b-2 border-primary pb-1" href="{{ route('home') }}">Home</a>
                @auth
                    <a class="text-white/70 text-sm font-medium hover:text-white transition-colors"
                    href="{{ route('favorites.index') }}">Favorites</a>
                    @if (auth()->user()->role === 'admin')
                        <a class="text-white/70 text-sm font-medium hover:text-white transition-colors" href="{{ route('admin') }}">Admin</a>
                    @endif
                @endauth
            </nav>
        </div>
