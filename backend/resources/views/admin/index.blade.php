...
<head>
    ...
    <title>Marine HQ Admin Dashboard - Night Mode</title>
    ...
    <style type="text/tailwindcss">
        :root {
            --steel-blue-dark: #0a111a;
            --steel-blue-bg: #0f172a;
            --marine-row-dark: #070b14;
            --marine-row-blue: #0f1c2e;
            --neon-blue: #00f2ff;
            --buster-red: #ff0033;
            --buster-glow: 0 0 15px rgba(255, 0, 51, 0.6);
        }
        ...
        .buster-call-btn {
            border: 1px solid var(--buster-red);
        }

        .buster-call-btn:hover {
            background-color: var(--buster-red);
            color: white;
        }
    </style>
...
<body class="bg-[var(--steel-blue-dark)] font-display text-white transition-colors duration-300">
        ...
        <aside class="w-64 bg-[#070b14] border-r border-white/10 flex flex-col justify-between fixed h-full z-20">
            <div class="flex flex-col gap-6 p-4">
                ...
                <nav class="flex flex-col gap-1">
                    <a class="flex items-center gap-3 px-3 py-2 rounded-xl bg-admin-primary/10 text-admin-primary border border-admin-primary/20 shadow-[0_0_10px_rgba(0,242,255,0.1)] transition-colors"
                        href="{{ route('admin') }}">
                        <span class="material-symbols-outlined "
                            style="font-variation-settings: 'FILL' 1">description</span>
                        <p class="text-sm font-bold leading-normal">Log Entries</p>
                    </a>
                    <a class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-white/5 transition-colors group"
                        href="{{ route('home') }}">
            ...
            <div class="px-8 pb-10 flex flex-col gap-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="flex flex-col gap-2 rounded-xl p-6 border border-white/10 bg-[#070b14]">
                        ...
                        <p class="text-white tracking-light text-2xl font-bold">{{ $questions->count() }}</p>
                    ...
                    <div class="flex flex-col gap-2 rounded-xl p-6 border border-white/10 bg-[#070b14]">
                        ...
                        <p class="text-white tracking-light text-2xl font-bold">{{ $users_count }}</p>
                        <p class="text-emerald-400 text-sm font-bold flex items-center gap-1">
                ...
                @if ($questions->count() > 0)
                    <div class="overflow-hidden rounded-xl border border-white/10 bg-white shadow-2xl">
                                ...
                                @foreach ($questions as $question)
                                    <tr
                                        ...
                                        <td class="px-6 py-4 text-sm text-slate-300">
                                            {{ $question->created_at->format('M d, Y') }}</td>
                                        <td class="px-6 py-4">
                                            <span
